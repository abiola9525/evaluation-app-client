// src/components/ModuleReviewForm.js
...
const ModuleReviewForm = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axiosInstance.post(`module/${moduleCode}/review/`, formData);
            toast.success(`Evaluation form for module ${moduleCode} has been filled successfully`);
            setTimeout(() => {
                navigate('/module');
    ...
    return (
        <Container className="mt-5 module-review-form-container">
            <h2 className="mb-4 text-center">Submit Review for Module {moduleCode}</h2>
            <Form onSubmit={handleSubmit}>
                <Form.Group controlId="moduleDetails">
                    <Form.Label>Module Details</Form.Label>
                    <Form.Control
                        type="text"
                        name="module_details"
                        placeholder="Enter module details"
                        value={formData.module_details}
